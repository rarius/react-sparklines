    ...
    _createClass(Sparklines, [{
        ...
        value: function render() {
            var _props = this.props,
                ...
                max = _props.max,
                min = _props.min,
                useSegments = _props.useSegments,
                warnThreshold = _props.warnThreshold,
                critThreshold = _props.critThreshold;
...
Sparklines.propTypes = {
    ...
    useSegments: _propTypes2.default.bool,
    warnThreshold: _propTypes2.default.number,
    critThreshold: _propTypes2.default.number
};
...
var flattenPoints = function flattenPoints(points) {
...
function segmentize(d) {
    ...
    var nextDiff = d.findIndex(function (e) {
...
var SparklinesLine = function (_React$Component) {
    ...
    _createClass(SparklinesLine, [{
        ...
        value: function render() {
            ...
            var linePoints = flattenPoints(points);

            var lineSegments = segmentize(points);
            ...
            if (useSegments) {
                return _react2.default.createElement(
                    ...
                    lineSegments.map(function (segment, i) {
...
function splitLine(p0, p1) {
    var t1 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -1;
    var t2 = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : -2;

    function getType(y) {
        var dec = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    if (y0 > y1) {
        ...
        lines.push(_extends({}, p0, { type: prevType }));
        if (y0 > t1 && t1 > y1) {
            var newY1 = t1;
            var newX1 = getX(x0, y0, x1, y1, t1);
        ...
        if (y0 > t2 && t2 > y1) {
            var newY2 = t2;
            var newX2 = getX(x0, y0, x1, y1, t2);
            lines.push({ x: newX2, y: newY2, type: prevType });
            prevType = "ok";
            lines.push({ x: newX2, y: newY2, type: prevType });
        }
        lines.push(_extends({}, p1, { type: prevType }));
        return lines;
    } else {
        // if (y0 < y1) {
        ...
        if (y0 < t2 && t2 < y1) {
            var _newY = t2;
            var _newX = getX(x0, y0, x1, y1, t2);
            lines.push({ x: _newX, y: _newY, type: prevType });
            prevType = "warning";
            lines.push({ x: _newX, y: _newY, type: prevType });
        }
        if (y0 < t1 && t1 < y1) {
            var _newY2 = t1;
            var _newX2 = getX(x0, y0, x1, y1, t1);
            lines.push({ x: _newX2, y: _newY2, type: prevType });
            prevType = "critical";
            lines.push({ x: _newX2, y: _newY2, type: prevType });
        }
        lines.push(_extends({}, p1, { type: prevType }));
...
exports.default = function (_ref) {
    var data = _ref.data,
        ...
        useSegments = _ref.useSegments,
        warnThreshold = _ref.warnThreshold,
        critThreshold = _ref.critThreshold;
    ...
    var modCrit = (max === min ? 1 : max - critThreshold) * vfactor + margin;
    var modWarn = (max === min ? 1 : max - warnThreshold) * vfactor + margin;
    ...
    if (useSegments) {
        ...
        for (var i = 0; i < points.length; i += 1) {
            if (i + 1 < points.length) {
                var temp = splitLine(points[i], points[i + 1], modCrit, modWarn);
                // console.log(temp);
